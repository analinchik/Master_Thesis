...
Check that the outliers have been removed. (Can't do this bcs you will just find more outliers.)

Distributions (with the outliers removed).
...
df <- ptms_mt %>% filter(mean_mt > 45 & mean_mt < 55) %>% group_by(Uniprot_entry_name) %>% summarise(sum_norm_counts = sum(counts_norm_abund_len)) %>% arrange(desc(sum_norm_counts)) 
...
svyranktest_result <- svyranktest(mean_mt~mod_group, design, test = "wilcoxon")
svyranktest_result
...
```{r}
# This is to check the proteins present in a particular half-life interval:
df <- p1 %>% filter(mean_mt > 48 & mean_mt < 52) %>% arrange(desc(sum)) 
...
oxPTMs_prot <- complete_mt %>% filter(unimod_id %in% oxPTMs$unimod_id & ptm_res %in% oxPTMs$AAs_list)
...
ggplot() +
  geom_violin(data = human_ptms_hl_mod_group_exclusive, aes(x = mod_group, y = mean_mt, weight = counts_norm_abund_len, fill = mod_group)) +
  geom_boxplot(data = proteins_exclusive, aes(x = mod_group, y = mean_mt, weight = sum), width = 0.1,  alpha = 0.5, position = position_dodge(width = 0.9)) +
  geom_blank(data = data.frame(mod_group = "Non-oxPTMs"), aes(x = mod_group, y = 50)) +
...
Kacylation_prot <- complete_mt %>% filter(unimod_id %in% lysine_acylation & ptm_res == 'K') 
...
```{r}
# This is to check the proteins present in a particular half-life interval:
df <- p1 %>% filter(mean_mt > 55 & mean_mt < 65) %>% arrange(desc(sum)) 
...
#| warning: false
#| message: false
AGEs_prot <- complete_mt %>% filter(unimod_id %in% AGE & ptm_res == 'K') 
...
General binning where we do not split by the type of modification:
...
mean_counts_per_mt_group <- df %>%
  ...
  distinct() %>% group_by(mt_group) %>% summarise(mean_ptms_group = mean(sum_ptms))

# Plot bar chart
ggplot(mean_counts_per_mt_group, aes(x = mt_group, y = mean_ptms_group)) +
  geom_bar(stat = "identity", position = 'dodge', fill = '#919191') +
...
Now need to split up by modification:
...
oxptms_prot <- human_complete %>% filter(unimod_id %in% oxPTMs$unimod_id) %>% inner_join(mt, by = join_by(LeadProt)) %>% filter(!ptm_class == 'Post-translational')
...
ggplot(mean_counts_per_mt_group, aes(x = mt_group, y = mean_ptms_group, fill = mod_group)) +
  geom_bar(stat = "identity", position = 'dodge') +
  scale_x_discrete(limits = c("40-45", "45-50", "50-55", "55-60", "60-65", "65-70")) +
  scale_y_continuous(limits = c(0,0.0065), labels = scales::label_number(scale = 1e4)) +
  scale_fill_manual(values = mod_group_colours) +
  labs(x = "Melting temperature (°C)",
       y = "Mean normalised counts (scaled by 1e4)") +
  theme_classic() +
...
ggplot(bins, aes(x = mean_mt)) +
  ...
  labs(x = 'Melting temperature (°C)', y = 'Protein counts') +
  facet_wrap(~bin) +
  theme_bw() +
  theme(
...
```{r}
#| echo: false
ks_test_result <- ks.test(bin1$mean_mt, bin3$mean_mt)
