...
Check that phosphorylation is the most abundant
...
ggplot() +
  ...
  geom_density(data = human_ptms_hl_short, aes(x = mean_hl_hours, weight = counts_norm_abund_len, fill = 'Short-lived proteins (dataset)'), alpha = 0.7, bw = 0.6) + 
...
acetyl_prot <- human_ptms_hl_mod_group %>% filter(mod_group == 'Acetyl') %>% dplyr::select(LeadProt) %>% distinct()
# those proteins that get methylated are not included in the non-meth section
human_ptms_hl_mod_group_exclusive <- human_ptms_hl_mod_group %>% filter(mod_group == 'Acetyl' | (mod_group == 'Non-Acetyl' & (!LeadProt %in% acetyl_prot$LeadProt)))
...
ggplot(human_ptms_hl_mod_group_exclusive, aes(x = mean_hl_hours, weight = counts_norm_abund_len, fill = mod_group)) +
  geom_density(alpha = 0.7, bw = 1) + 
  labs(x = 'Mean half-life (hours)', y = 'Counts')+
  scale_fill_manual(values = c('Non-ubiquitinated' = "#5DB7B1", 'Ubiquitinated' = '#FB8500')) +
  theme_classic() +
  theme(legend.title = element_blank(),
        legend.text = element_text(size = 12, family = "Lato"),
        legend.position = c(0.8,0.9), 
...
mean_hours_per_hl_group <- df %>%
  ...
  filter(!mod_group == '-') %>%
  distinct() %>% group_by(mod_group, hl_group) %>% summarise(mean_ptms_group = log10(mean(sum_ptms)*10000))
...
```{r}
human_ptms_hl_mod_group <- human_ptms_hl_short
human_ptms_hl_mod_group$mod_group <- if_else(human_ptms_hl_mod_group$unimod_id == 34, 'Methylated', 'Unmethylated')
human_ptms_hl_mod_group <- human_ptms_hl_mod_group 

human_ptms_hl_mod_group$mod_group <- factor(human_ptms_hl_mod_group$mod_group, levels = c("Unmethylated", "Methylated"))
...
# those proteins that get methylated are not included in the non-meth section
human_ptms_hl_mod_group_exclusive <- human_ptms_hl_mod_group %>% filter(mod_group == 'Methylated' | (mod_group == 'Unmethylated' & (!LeadProt %in% meth_prot$LeadProt)))

ggplot(human_ptms_hl_mod_group_exclusive, aes(x = mean_hl_hours, weight = norm_counts, fill = mod_group)) +
  geom_density(alpha = 0.7, bw = 1) + 
  labs(x = 'Mean protein half-life (hours)', y = 'Density')+
  scale_fill_manual(values = c('Unmethylated' = "#5DB7B1", 'Methylated' = '#FB8500')) +
  theme_classic() +
  theme(legend.title = element_blank(),
        legend.text = element_text(size = 12, family = "Lato"),
...
AGE <- c(6, 907, 378)
AGE_lysine <- c(860,859)
#oxPTMs
a <- human_complete %>% filter(unimod_id %in% c(lysine_acylation, AGE, AGE_lysine, oxPTMs$unimod_id)) %>%
  mutate(mod_group = case_when(
    (unimod_id %in% AGE_lysine & ptm_res == 'K') ~ 'AGE_lysine',
    unimod_id %in% AGE ~ 'AGE',
    (unimod_id == 34 & ptm_class == 'Post-translational') ~'Methylation',
    (unimod_id %in% lysine_acylation & ptm_res == 'K') ~ 'lysine_acylation',
    (unimod_id %in% oxPTMs$unimod_id & ptm_res %in% oxPTMs$AAs_list) ~ 'oxPTMs',
    TRUE ~ "-"
  ))  %>% filter(!mod_group == '-') %>% inner_join(short_lived_proteins_hl, by = join_by(LeadProt))
...
mean_hours_per_hl_group <- df %>%
  group_by(Uniprot_entry_name, mod_group) %>%
  summarize(sum_ptms = sum(counts_norm_abund_len), hl_group) %>% 
  distinct() %>% group_by(mod_group, hl_group) %>% summarise(mean_ptms_group = log10(mean(sum_ptms)*100000))

mod_group_colours <- c('oxPTMs' = '#EA7317', 'lysine_acylation' = '#FFB703', 'AGE' = '#2364AA')
...
ggplot() +
  ...
  geom_density(data = human_ptms_hl_long, aes(x = mean_hl_hours, weight = norm_counts, fill = 'Long-lived proteins (dataset)'), alpha = 0.7, bw = 8) + 
  labs(x = 'Mean half-lives (hours)', y = 'Density') +
  theme_classic() +
...
```{r}
human_ptms_hl_mod_group <- human_ptms_hl_long
human_ptms_hl_mod_group$mod_group <- if_else(human_ptms_hl_long$unimod_id == 34, 'Meth', 'Non-Meth')
...
```{r}
human_ptms_hl_mod_group <- human_ptms_hl_long
human_ptms_hl_mod_group$mod_group <- if_else(human_ptms_hl_long$unimod_id == 1, 'Acetyl', 'Non-Acetyl')
...
human_complete_hl_long_ageing <- human_complete_hl_long %>% filter(Uniprot_entry_name %in% genage$uniprot)
```
...
a <- human_complete %>% filter(unimod_id %in% c(lysine_acylation, AGE, AGE_lysine, oxPTMs$unimod_id)) %>%
  mutate(mod_group = case_when(
    ...
    (unimod_id %in% oxPTMs$unimod_id & ptm_res %in% oxPTMs$AAs_list) ~ 'oxPTMs',
    TRUE ~ "-"
  ))  %>% filter(!mod_group == '-') %>% inner_join(long_lived_proteins_hl, by = join_by(LeadProt))
