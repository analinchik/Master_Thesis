...
s <- human_complete %>% 
  ...
  summarise(sum_counts = sum(total_counts), abundance) %>%
  distinct() 

ggplot(s, aes(x = abundance, y = sum_counts)) +
  geom_col() +
  scale_x_continuous(limits = c(0,1)) +
  scale_y_continuous(limits = c(0,7000))

cor(s$sum_counts, s$abundance)
```

```{r}
s <- human_complete %>% 
  group_by(Uniprot_entry_name) %>% 
  summarise(sum_counts = sum(total_counts), Length) %>%
  distinct() 
ggplot(aes(x = Length, y = sum_counts)) +
  geom_col() +
  scale_x_continuous(limits = c(0,100))

cor(s$sum_counts, s$Length)
```

```{r}
s <- human_complete %>% 
  group_by(project_id) %>% 
  summarise(sum_counts = sum(total_counts), raw_files_counts) %>%
  distinct() 

ggplot(aes(x = raw_files_counts, y = sum_counts)) +
  geom_col() +
  scale_x_continuous(limits = c(0,100))


cor(s$sum_counts, s$raw_files_counts)
```
...
long_lived_proteins_hl <- read.csv(file = '/Users/anastasialinchik/Desktop/Thesis/DATA/generated/long_proteins_hl_grouped.tsv', sep = '\t', header = T) %>% filter(mean_hl_hours > 40)
