---
title: "PTMs and protein half-lives"
format: html
editor: visual
---
...
```{r}
short_lived_proteins_hl <- read_excel(path = '/Users/anastasialinchik/Desktop/Thesis/DATA/short_lived_proteins_hl.xlsx', sheet = 'HEK293T') %>% dplyr::select(`Uniprot ID`, `Half-life`)
long_lived_proteins <- # use the code that you already have
# mouse_proteins_hl <- 
```
...
ggplot() +
  ...
  geom_density(data = human_ptms_hl, aes(x = mean_hl_hours, weight = total_counts, fill = 'Proteins in the dataset'), alpha = 0.7, bw = 0.4) + 
...
This is with the alternative half-lives:

```{r}

long_proteins_hl_df <- long_proteins_hl %>% filter(mean_hl_hours < 200)

ggplot() +
  geom_density(data = hl, aes(x = mean_hl_hours, fill = 'Half-lives in papers'), alpha = 0.7, bw = 0.4) +
  geom_density(data = long_proteins_hl_df, aes(x = mean_hl_hours, fill = 'Proteins in the dataset'), alpha = 0.7, bw = 0.4) + 
  labs(title = 'Distributions of protein half-lives', x = 'Mean half-lives (hours)', y = 'Density') +
  theme_classic() +
  scale_fill_manual(values = c("#5DB7B1", '#FB8500'))  # Manually specify fill colors
```

Both hl dataframes complement each other

```{r}
hl_and_long <- rbind(hl, long_proteins_hl) %>% group_by(LeadProt) %>% 
  summarise(mean_hl_hours = mean(mean_hl_hours))

human_ptms_hl <- inner_join(human_ptms, hl_and_long, by = join_by(LeadProt == LeadProt)) %>% filter(mean_hl_hours < 200)

ggplot() +
  geom_density(data = hl_and_long, aes(x = mean_hl_hours, fill = 'Half-lives in papers'), alpha = 0.7, bw = 8) +
  geom_density(data = df, aes(x = mean_hl_hours, fill = 'Proteins in the dataset'), alpha = 0.7, bw = 8) + 
  labs(title = 'Distributions of protein half-lives', x = 'Mean half-lives (hours)', y = 'Density') +
  theme_classic() +
  scale_fill_manual(values = c("#5DB7B1", '#FB8500'))  # Manually specify fill colors
```

Now it looks like a proper subset.

Nonetheless, the change in PTM abundances can still be investigated.
...
ggplot() +
  geom_density(data = no_pho_prot, aes(x = mean_hl_hours, fill = 'no_pho_prot'), alpha = 0.7, bw = 2) +
  geom_density(data = pho_no_age_prot, aes(x = mean_hl_hours, fill = 'pho_no_age_prot'), alpha = 0.7, bw = 2) +
   geom_density(data = pho_age_prot, aes(x = mean_hl_hours, fill = 'pho_age_prot'), alpha = 0.7, bw = 2) +
  labs(title = 'Distributions of protein half-lives', x = 'Mean half-lives (hours)', y = 'Density') +
  theme_classic() +
...
oxPTMs_hl <- human_ptms_hl %>% 
  ...
  group_by(LeadProt) %>%
  summarise(mean_hl_hours, sum_oxPTMs = sum(norm_counts)) %>%
  distinct() %>% inner_join(iupred_scores_ratios, by = join_by(LeadProt == UniProt_ID))
