...
df_melt <- melt(df_centers[,1:12], id.vars = "cluster")
...
df <- data %>% dplyr::select("Uniprot_entry_name", "sum_acetyl_cyto_old", "sum_acetyl_cyto_young",
  ...
  "sum_phospho_cyto_old", "sum_phospho_cyto_young",
  "sum_ubiq_cyto_old", "sum_ubiq_cyto_young",
  "sum_meth_cyto_old", "sum_meth_cyto_young",  "mean_hl_hours", "mean_mt", "Ratio_Above_0.5")
column_names <- c("sum_acetyl_cyto_old", "sum_acetyl_cyto_young",
  ...
  "sum_phospho_cyto_old", "sum_phospho_cyto_young",
  "sum_ubiq_cyto_old", "sum_ubiq_cyto_young",
  "sum_meth_cyto_old", "sum_meth_cyto_young", "mean_hl_hours", "mean_mt", "ratio")
rows <- df$Uniprot_entry_name
df <- data.frame(df, row.names = rows)
df <- df %>% select(-Uniprot_entry_name)
names(df) <- column_names
...
df_centers <- df_centers %>% filter(cluster_size >10)
# Reshape the data frame for ggplot2
df_melt <- melt(df_centers[,1:18], id.vars = "cluster")
...
#| echo: false
# Initial left join with leadprot_uniprot and the sum_acetyl variables
data <- left_join(leadprot_uniprot, mt, by = join_by(LeadProt))
#data <- left_join(data, mt, by  = join_by(LeadProt))
data <- left_join(data, long_lived_proteins_hl, by  = join_by(LeadProt))
data <- left_join(data, iupred_scores, by  = join_by('LeadProt' == 'UniProt_ID'))
```

```{r}
#| echo: false
df <- data %>% dplyr::select("Uniprot_entry_name", "mean_hl_hours", "mean_mt", "Ratio_Above_0.5")
column_names <- c("mean_hl_hours", "mean_mt", "ratio")
rows <- df$Uniprot_entry_name
df <- data.frame(df, row.names = rows)
df <- df %>% select(-Uniprot_entry_name)
names(df) <- column_names
...
cluster8 <- df_with_clusters[df_with_clusters$Cluster == 8, ] %>%
  rownames_to_column(var = "UniProt")
cluster10 <- df_with_clusters[df_with_clusters$Cluster == 10, ] %>%
  rownames_to_column(var = "UniProt")
```
...
write.table(cluster7$UniProt, file = '/Users/anastasialinchik/Downloads/cluster7.tsv', row.names = F, sep="\t", quote = F)
write.table(cluster8$UniProt, file = '/Users/anastasialinchik/Downloads/cluster8.tsv', row.names = F, sep="\t", quote = F)
write.table(cluster10$UniProt, file = '/Users/anastasialinchik/Downloads/cluster10.tsv', row.names = F, sep="\t", quote = F)
```
...
# Define cluster names (make sure the order matches the clusters)
df_centers$cluster_name <- c('Cluster 1 (56)', 'Cluster 2 (71)', 'Cluster 3 (75)', 'Cluster 4 (56)', 'Cluster 5 (2)', 'Cluster 6 (35)', 'Cluster 7 (51)', 'Cluster 8 (136)', 'Cluster 9 (25)', 'Cluster 10 (75)')

# Convert cluster to numeric for sorting
df_centers$cluster <- as.numeric(df_centers$cluster)

# Filter data by cluster size and keep the original order
df_centers <- df_centers %>%
  filter(cluster_size > 25) %>%
  arrange(cluster)  # Ensure cluster is treated as numeric for correct ordering

# Convert cluster_name to a factor with levels ordered by 'cluster'
df_centers$cluster_name <- factor(df_centers$cluster_name, levels = df_centers$cluster_name[order(df_centers$cluster)])

# Reshape the data frame for ggplot2
df_melt <- melt(df_centers[, c(1:3, 6)], id.vars = "cluster_name")

# Create the heatmap using ggplot2
ggplot(df_melt, aes(y = variable, x = cluster_name, fill = value)) +
  geom_tile() +
  scale_y_discrete(labels = c('Half-life \n(hours)', 'Melting \ntemperature \n(°C)', 'IUPred3 score')) +  # Custom labels for y-axis
  ...
  labs(
    ...
    fill = NULL) +
  theme_minimal() +
  theme(axis.text.x = element_text(size = 14, angle = 45, family = "Lato", vjust = 0.9, hjust = 0.95),
        axis.text.y = element_text(size = 14, family = "Lato"),
        axis.title.y = element_text(size = 14, family = "Lato"),
