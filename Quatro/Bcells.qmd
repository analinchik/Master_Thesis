...
histones
human_ptms_cyto_old %>% filter(LeadProt %in% histones$Entry)
human_ptms_cyto_young %>% filter(LeadProt %in% histones$Entry)
...
(maybe it would be better to look a)
...
Which PTMs are enriched in old cells compared to new?
...
```{r}
write.table(df3$Uniprot_entry_name, file = '/Users/anastasialinchik/Downloads/enriched.tsv', row.names = F, sep="\t", quote = F)
```

This list can be used for DAVID to check for significant enrichment of GO terms.

# K-means prep

Need the following columns in a dataframe:

-   ageing score

-   \# of PTM modification counts in cyto young

-   \# of PTM modification counts in cyto old

-   \# of PTM modification counts in nuc young

-   \# of PTM modification counts in nuc old

-   percentage of all modifications that are oxPTMs

-   percentage of all modifications that are AGEs

-   percentage of all modifications that are K acylations

-   percentage of PTMs that are methylations
...
totals <- human_complete %>% group_by(Uniprot_entry_name) %>%
  summarise(totals = sum(counts_norm_abund_len))
perc_oxPTMs <- inner_join(sum_oxPTMs, totals, by = join_by(Uniprot_entry_name)) %>% mutate(sum_oxPTMs/totals)
...
perc_AGEs <- inner_join(sum_AGEs, totals, by = join_by(Uniprot_entry_name)) %>% mutate(sum_AGEs/totals)

# PTMs numbers

sum_cyto_young <- human_ptms_cyto_young %>% group_by(Uniprot_entry_name) %>%
  summarise(sum_cyto_young = sum(counts_norm_abund_len))
sum_cyto_old <- human_ptms_cyto_old %>% group_by(Uniprot_entry_name) %>%
  summarise(sum_cyto_old = sum(counts_norm_abund_len))

cyto <- inner_join(sum_cyto_young, sum_cyto_old, by = join_by(Uniprot_entry_name))

sum_nuc_young <-human_ptms_nuc_young %>% group_by(Uniprot_entry_name) %>%
  summarise(sum_nuc_young = sum(counts_norm_abund_len))
sum_nuc_old <- human_ptms_nuc_old %>% group_by(Uniprot_entry_name) %>%
  summarise(sum_nuc_old = sum(counts_norm_abund_len))

nuc <- inner_join(sum_nuc_young, sum_nuc_old, by = join_by(Uniprot_entry_name))

PTMs <- inner_join(nuc, cyto, by = join_by(Uniprot_entry_name))
# ageing score
...
age_proteins$score <- age_scores$preds
names(age_proteins) <- c('Uniprot_entry_name', 'GenAge', 'score')

# hl
Bcells_hl_uni <- inner_join(Bcells_hl, leadprot_uniprot, by = join_by('LeadProt'))
...
data <- inner_join(perc_oxPTMs, PTMs, by = join_by(Uniprot_entry_name))
data <- inner_join(data, age_proteins, by = join_by(Uniprot_entry_name))
data <- inner_join(data, Bcells_hl_uni, by = join_by(Uniprot_entry_name))
data <- inner_join(data, mt, by = join_by(LeadProt))
data <- inner_join(data, iupred_scores, by = join_by(LeadProt == 'UniProt_ID'))
```
...
human_ptms_cyto_old_hl <- human_ptms_cyto_old %>% 
  inner_join(Bcells_hl, by = join_by(LeadProt))
human_ptms_cyto_young_hl <- human_ptms_cyto_young %>% 
  inner_join(Bcells_hl, by = join_by(LeadProt))

human_ptms_nuc_old_hl <- human_ptms_nuc_old %>% 
  inner_join(Bcells_hl, by = join_by(LeadProt))
human_ptms_nuc_young_hl <- human_ptms_nuc_young %>% 
  inner_join(Bcells_hl, by = join_by(LeadProt))
```
...
```{r}
human_cyto_old <- human_complete %>%
  filter(project_id == 'PXD006570') 

human_cyto_young <- human_complete %>%
...
```{r}
all_chaperones <- read_excel(path = '/Users/anastasialinchik/Desktop/Thesis/DATA/all_chaperones.xlsx', sheet = 'BREHME_TABLE S2A', skip = 8, col_names = T) %>% dplyr::select(`Entrez-ID`, `Uniprot`) %>% separate_rows(Uniprot, sep = "; ") 
...
Does the mean sum of the modifications in young vs old significant?

-   the total modificaitons are significantly different but not post-translational

```{r}
df1 <- human_ptms_cyto_young %>% filter(Uniprot_entry_name %in% all_chaperones$Uniprot) %>% 
  group_by(Uniprot_entry_name) %>%
  summarise(sum_mod = sum(counts_norm_abund_len)) %>% mutate(group = 'young')
df2 <- human_ptms_cyto_old %>% filter(Uniprot_entry_name %in% all_chaperones$Uniprot) %>% filter(Uniprot_entry_name %in% df1$Uniprot_entry_name) %>%
  group_by(Uniprot_entry_name) %>%
  summarise(sum_mod = sum(counts_norm_abund_len)) %>% mutate(group = 'old')

wilcox.test(df1$sum_mod, df2$sum_mod)
```

```{r}

df3 <- rbind(df1,df2)
ggplot(df3, aes(x = group, y = sum_mod)) +
  geom_boxplot(outliers = F)
```

```{r}
df1 <- human_ptms_nuc_young %>% filter(Uniprot_entry_name %in% all_chaperones$Uniprot) %>% 
  group_by(Uniprot_entry_name) %>%
  summarise(sum_mod = sum(counts_norm_abund_len)) %>% mutate(group = 'young')
df2 <- human_ptms_nuc_old %>% filter(Uniprot_entry_name %in% all_chaperones$Uniprot) %>% filter(Uniprot_entry_name %in% df1$Uniprot_entry_name) %>%
  group_by(Uniprot_entry_name) %>%
  summarise(sum_mod = sum(counts_norm_abund_len)) %>% mutate(group = 'old')
