...
hepatocytes1 <- subset(long_lived_proteins_hl, select = c(1,14,15))
hepatocytes2 <- subset(long_lived_proteins_hl, select = c(1,17,18))
hepatocytes1 <- na.omit(hepatocytes1)
hepatocytes2 <- na.omit(hepatocytes2)
names(hepatocytes1) <- c('gene_name', 'half_life', 'quality')
names(hepatocytes2) <- c('gene_name', 'half_life', 'quality')
hepatocytes <- rbind(hepatocytes1, hepatocytes2)
quality <- if_else(hepatocytes$quality == 'good', 1, 0)
hepatocytes$quality <- quality
# only include cells with good quality
hepatocytes <- hepatocytes %>% filter(quality == 1)


hepatocytes <- hepatocytes %>% group_by(gene_name) %>%
  summarise(mean_hl = mean(half_life)) 

hepatocytes$cell_type <- 'hepatocytes' 

cell_types_hl <- rbind(Bcells, NKcells, hepatocytes, hepatocytes)
